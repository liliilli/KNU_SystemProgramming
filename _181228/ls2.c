 ...
 */
int TryGetStatusInfo(const char* iPath, struct stat** iPPtrStatus);
...
void ProcessLs(const char* iDirName)
{
	...
	{
		if (TryGetStatusInfo(ptrDirent->d_name, &ptrInfoBuf) == -1)
		{
			fprintf(
					stderr, "ls2 : Failed to retrieve information of file %s\n", 
					ptrDirent->d_name);
			continue;
		}
...
const char* GetUserString(uid_t iUid)
{
	return getpwuid(iUid)->pw_name;
}

const char* GetGroupString(gid_t iGid)
{
	return getgrgid(iGid)->gr_name;
}
